...
function validateTaskInput(taskText) {
  ...
  if (trimmed.length > 100) {
    return {
      valid: false,
      message: "Task is too long! Keep it under 100 characters.",
    };
  }
...
function addTask(taskText, priority) {
  try {
    ...
    console.log(
      `Task added successfully. Total tasks: ${todoState.tasks.length}`
    );
  } catch (error) {
...
function deleteAllCompletedTasks() {
  ...
  if (
    confirm(
      `Are you sure you want to delete ${completedTasks.length} completed task${
    ...
    updateDisplay();
    alert(
      `Deleted ${completedTasks.length} completed task${
        completedTasks.length !== 1 ? "s" : ""
      }!`
    );
  }
...
function clearAllTasks() {
  ...
  if (
    confirm(
      `Are you sure you want to delete ALL ${todoState.tasks.length} tasks? This cannot be undone!`
    )
  ) {
